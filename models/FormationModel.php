...
class FormationModel extends SQL
{
    ...
    {
        $stmt = $this->pdo->query("SELECT * FROM formation where VISIBILITEPUBLIC = 1 and formation.DATEVISIBILITE <= now()");
        return $stmt->fetchAll(\PDO::FETCH_ASSOC);
    }

    function getVideos()
    {
        $stmt = $this->pdo->query("SELECT * FROM formation where formation.DATEVISIBILITE <= now()");
        return $stmt->fetchAll(\PDO::FETCH_ASSOC);
    }
